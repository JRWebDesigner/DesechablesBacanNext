...
export default function DelSur(){
  ...
  return(
      <>
				<h1 data-aos='fade-up' className="text-center font-bold text-3xl mt-20">Del Sur</h1>
        <div className="flex flex-wrap justify-around items-center gap-5 m-10">
				{
        products.filter(prod=>prod.mark.toUpperCase()==='DEL SUR').length== 0 ? (
          <div className='flex justify-center items-center text-2xl'>Se nos acabo los Productos de la marca Del Sur</div>
        ):(products
      .filter((prod) => prod.mark.toUpperCase() === 'DEL SUR')
      .map((prod) => (
        <div 
          key={prod.id} 
