...
export default function PastenPlast(){
    return(
      <>
				<h1 data-aos='fade-up' className="text-center font-bold text-3xl mt-20">Pasten Plast</h1>
        <div className="flex flex-wrap justify-around items-center gap-5 m-10">
				{
        products.filter(prod=>prod.mark.toUpperCase()==='PASTENPLAST').length== 0 ? (
          <div className='flex justify-center items-center text-2xl'>Se nos acabo los Productos de la marca Pasten Plast</div>
        ):(products
      .filter((prod) => prod.mark.toUpperCase() === 'PASTENPLAST')
      .map((prod) => (
        <div 
          key={prod.id} 
